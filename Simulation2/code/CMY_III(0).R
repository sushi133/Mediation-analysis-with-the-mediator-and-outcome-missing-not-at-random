...
m[R_m==0] <- NA
y[R_y==0] <- NA
data<-as.data.frame(cbind(t,m,x,y,R_m,R_y,t_m,t_y))
...
#arguments: m,t,x: observed data, y: imputed value for y
#output: weight of y 
...
#arguments: t,x: observed data, y: imputed value for y, m: imputed value for m
#output: weight of (y,m) 
cmy_weight <- function(y,m,t,x){
  #joint dist  
  p <- 
    (1/(emsd_y*sqrt(2*pi)))*exp((-0.5*((y-(emb_0+emb_m*m+emb_t*t+emb_mt*m*t+emb_x*x))/emsd_y)^2))*
    (1/(emsd_m*sqrt(2*pi)))*exp((-0.5*((m-(ema_0+ema_t*t+ema_x*x))/emsd_m)^2))*
...
while (sum(abs(Q[[k]]-Q[[k-1]]))/sum(Q[[k-1]])>=1e-5) {
    ...
    #missing m and y
    dat3$wt<-cmy_weight(dat3$y,dat3$m,dat3$t,dat3$x)
...
write.xlsx(timediff,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation2/Compute_time(hours).xlsx',
           row.names = FALSE,sheetName="CMY_III(0)",append=TRUE)

write.xlsx(save,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation2/CMY_III(0).xlsx',row.names = FALSE)

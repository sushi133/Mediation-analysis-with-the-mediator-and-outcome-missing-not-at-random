...
while (sum(abs(Q[[k]]-Q[[k-1]]))/sum(Q[[k-1]])>=1e-5) {
  ...
  #update parameters
  emm1<-glm(y~m+t+m*t+x,family = binomial(link='logit'),weights=wt,data=dat,subset=(R_m==1))
...
write.xlsx(save,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation2/BMY_I(0).xlsx',row.names = FALSE)

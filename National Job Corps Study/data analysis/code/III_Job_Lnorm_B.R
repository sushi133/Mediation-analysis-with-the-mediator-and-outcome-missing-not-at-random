...
#purpose: estimate parameters from 500 bootstrap samples
#argument: bootstrap sample
#output: estimated parameters from 500 bootstrap samples
func<-function(i){
...
ccm11<-glm(I_Y~M+Z+M*Z+x1+x2+x3+x4+x5+x6+x7,family=binomial(link='logit'),data=dat,subset=(R_m==1 & R_y==1))
ccm1<-lm(log(Y)~M+Z+M*Z+x1+x2+x3+x4+x5+x6+x7,data=dat,subset=(R_m==1 & R_y==1 & I_Y==1))
...
#mice for MAR
dat_mi<-subset(dat, select = c(Y,M,Z,x1,x2,x3,x4,x5,x6,x7,R_y,R_m))
...
return(matrix(c(cca_0,mia_0,ema_0,
                cca_t,mia_t,ema_t,
                ...
                ccPDE,miPDE,emPDE,
                ccPIE,miPIE,emPIE,
                ccTDE,miTDE,emTDE),byrow=T,106,3))
...
write.xlsx(timediff,'/Users/sushi5824907/Desktop/Mediation/JASA/Data_Analysis/Compute_time(hours).xlsx',
           row.names = FALSE,sheetName="III_Job_Lnorm_B",append=TRUE)

#save param
write.xlsx(save,'/Users/sushi5824907/Desktop/Mediation/JASA/Data_Analysis/III_Job_Lnorm_B_Param.xlsx',row.names = FALSE)
param <- read.xlsx('/Users/sushi5824907/Desktop/Mediation/JASA/Data_Analysis/III_Job_Lnorm_B_Param.xlsx', 1)

#summary param
results<-array(as.numeric(unlist(param)), dim = c(106, 3, runs))
#summary table
tab<-cbind(t(rbind(round(apply(results,c(1,2), mean),3)[1:106,1],
                   ...
                   round(apply(results,c(1,2), mean)+apply(results,c(1,2), sd)*qnorm(0.975),3)[1:106,1])),
           t(rbind(round(apply(results,c(1,2), mean),3)[1:106,2],
                   round(apply(results,c(1,2), sd),3)[1:106,2],
                   round(apply(results,c(1,2), mean)-apply(results,c(1,2), sd)*qnorm(0.975),3)[1:106,2],
                   round(apply(results,c(1,2), mean)+apply(results,c(1,2), sd)*qnorm(0.975),3)[1:106,2])),
           t(rbind(round(apply(results,c(1,2), mean),3)[1:106,3],
                   round(apply(results,c(1,2), sd),3)[1:106,3],
                   round(apply(results,c(1,2), mean)-apply(results,c(1,2), sd)*qnorm(0.975),3)[1:106,3],
                   round(apply(results,c(1,2), mean)+apply(results,c(1,2), sd)*qnorm(0.975),3)[1:106,3])))

library("xlsx")
write.xlsx(tab,'/Users/sushi5824907/Desktop/Mediation/JASA/Data_Analysis/III_Job_Lnorm_B.xlsx',row.names = FALSE)

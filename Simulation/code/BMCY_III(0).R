...
cy_weight<-function(y,m,t,x){
  ...
  #proposed dist
  h <- 
    (1/(psd_y*sqrt(2*pi)))*exp((-0.5*((y-(ccb_0+ccb_m*m+ccb_t*t+ccb_mt*m*t+ccb_x*x))/psd_y)^2))
...
while (sum(abs(Q[[k]]-Q[[k-1]]))/sum(Q[[k-1]])>=1e-5) {
  ...
  dat2$wt<-cy_weight(dat2$y,dat2$m,dat2$t,dat2$x)
  dat2<-dat2 %>% group_by(seq) %>% mutate(wt=wt/sum(wt))
  
  #missing m and y
  dat3<-subset(dat, R_m==0 & R_y==0)
  dat3$wt<-cy_weight(dat3$y,dat3$m,dat3$t,dat3$x)
  dat3<-dat3 %>% group_by(seq,m) %>% mutate(wt=wt/sum(wt))
  dat3$wt<-my_weight(dat3$m,dat3$t,dat3$x)*dat3$wt
...
write.xlsx(timediff,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation/Compute_time(hours).xlsx',
           row.names = FALSE,sheetName="BMCY_III(0)",append=TRUE)

write.xlsx(save,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation/BMCY_III(0).xlsx',row.names = FALSE)

...
m4<-glm(R_y~R_m+t+x,family=binomial(link='logit'),data=dat)
tb_0<-m1$coef[1]
tb_m<-m1$coef[2]
tb_t<-m1$coef[3]
tb_x<-m1$coef[4]
...
while (sum(abs(Q[[k]]-Q[[k-1]]))/sum(Q[[k-1]])>=1e-5) {
  ...
  emm4<-glm(R_y~R_m+t+x,family = "binomial",weights=wt,data=dat)
  emb_0<-emm1$coef[1]
  emb_m<-emm1$coef[2]
  emb_t<-emm1$coef[3]
  emb_x<-emm1$coef[4]
  ...
  Q[[k]]<-c(emb_0,emb_m,emb_t,emb_mt,emb_x,emsd_y,ema_0,ema_t,ema_x,emsd_m,emc_0,emc_m,emc_t,emc_x,emd_0,emd_rm,emd_t,emd_x)
...
write.xlsx(timediff,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation/Compute_time(hours).xlsx',
           row.names = FALSE,sheetName="CMY_II(0)",append=TRUE)

write.xlsx(save,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation/CMY_II(0).xlsx',row.names = FALSE)

...
m_weight<-function(y,m,t,x){
  ...
  cond_prob_1<-
    ...
    (exp(emd_0+emd_m*1+emd_t*t+emd_x*x)/(1+exp(emd_0+emd_m*1+emd_t*t+emd_x*x)))
  ...
  cond_prob_0<-
    ...
    (exp(emd_0+emd_m*0+emd_t*t+emd_x*x)/(1+exp(emd_0+emd_m*0+emd_t*t+emd_x*x)))
...
my_weight<-function(m,t,x){
  ...
  cond_prob_1<-
    (exp(ema_0+ema_t*t+ema_x*x)/(1+exp(ema_0+ema_t*t+ema_x*x)))*
    (1/(1+exp(emc_0+emc_m*1+emc_t*t+emc_x*x)))*
    (1/(1+exp(emd_0+emd_m*1+emd_t*t+emd_x*x)))
  
  cond_prob_0<-
    (1/(1+exp(ema_0+ema_t*t+ema_x*x)))*
    (1/(1+exp(emc_0+emc_m*0+emc_t*t+emc_x*x)))*
    (1/(1+exp(emd_0+emd_m*0+emd_t*t+emd_x*x)))
...
Q[[2]] <-c(emb_0,emb_m,emb_t,emb_mt,emb_x,emsd_y,ema_0,ema_t,ema_x,emc_0,emc_m,emc_t,emc_x,emd_0,emd_m,emd_t,emd_x)
k<-2
while (sum(abs(Q[[k]]-Q[[k-1]]))/sum(Q[[k-1]])>=1e-5) {
  ...
  #update parameters
  emm1<-lm(y~m+t+m*t+x,weights=wt,data=dat,subset=(R_y==1))
  ...
  emb_x<-emm1$coef[4]
  emb_mt<-emm1$coef[5]
  emsd_y<-sqrt(sum(subset(dat,R_y==1)$wt*(emm1$residuals)^2)/(sum(subset(dat,R_y==1)$wt)-emm1$rank))
  ema_0<-emm2$coef[1]
  ema_t<-emm2$coef[2]
  ema_x<-emm2$coef[3]
  emc_0<-emm3$coef[1]
  ...
  Q[[k]]<-c(emb_0,emb_m,emb_t,emb_mt,emb_x,emsd_y,ema_0,ema_t,ema_x,emc_0,emc_m,emc_t,emc_x,emd_0,emd_m,emd_t,emd_x)
...
write.xlsx(timediff,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation/Compute_time(hours).xlsx',
           row.names = FALSE,sheetName="BMCY_IV(0)",append=TRUE)

write.xlsx(save,'/Users/sushi5824907/Desktop/Mediation/JASA/Simulation/BMCY_IV(0).xlsx',row.names = FALSE)
